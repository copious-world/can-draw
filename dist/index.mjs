...
function instance($$self, $$props, $$invalidate) {
	...
	g_commander.subscribe(command => {
		...
		if (!ctxt && the_canvas) {
			$$invalidate(9, ctxt = the_canvas.getContext("2d"));
		...
		if (command.shape !== undefined) {
			...
			$$invalidate(8, mouse_to_shape = drawit.mouse_in_shape(pars));
		}

		$$invalidate(7, selected = drawit.selected_object());
	});
	...
	$$self.$$set = $$props => {
		...
		if ('mouse_to_shape' in $$props) $$invalidate(8, mouse_to_shape = $$props.mouse_to_shape);
	};

	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*the_canvas, drawit, ctxt, width, height*/ 1603) {
			if (the_canvas && !drawit) {
				$$invalidate(9, ctxt = the_canvas.getContext("2d"));
				$$invalidate(10, drawit = new DrawTools(ctxt, width, height));
			}
		}
	};
